...
BoolVector::BoolVector(const int len, bool var)
{
	if (len <= 0)
	{
		std::cout << "wrong input len in BoolVector::BoolVector";
		exit(-1);
	}
	_len = len;
	_cells = (len + 7) / 8;
...
void BoolVector::inverseComponent(const int pos)
{
	if (pos < 0 || pos >= _len)
	{
		std::cout << "Wrong input pos in BoolVector::inverseComponent";
		exit(-1);
	}
	int cell = pos / 8;
	char mask = 1 << (pos % 8);
...
void BoolVector::setComponent(const int pos, const bool value)
{
	if (pos < 0 || pos >= _len)
	{
		std::cout << "Wrong input pos in BoolVector::setComponent";
		exit(-1);
	}
	int cell = pos / 8;
	char mask = 1 << (pos % 8);
...
bool BoolVector::operator==(const BoolVector& vec)
{
	if (vec._len != _len) return false;
	for (int i = 0; i < _len; i++)
	{
		if (operator[](i) != vec[i]) return false;
	}
	return true;
}
